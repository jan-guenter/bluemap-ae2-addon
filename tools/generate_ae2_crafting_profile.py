...
"""Generate or check the exact AE2 19.2.17 M3d crafting route."""
...
def verify_partition_hashes(manifest: bytes) -> None:
    ...
    for label, content, expected in checks:
        if hashlib.sha256(content).hexdigest() != expected:
            raise ValueError(f"M3d {label} changed")
...
def main() -> int:
    ...
    if stale:
        raise ValueError(f"generated M3d outputs are stale: {stale}")
    print(
        ("Verified" if args.check else "Generated")
        + " exact AE2 19.2.17 M3d crafting route: 8 blocks, 15 textures, "
        "30 disjoint resources and 29 known compatible extension connectors."
    )
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"AE2 crafting profile generation failed: {error}", file=sys.stderr)
        sys.exit(1)
