...
archive_temp="$(mktemp -d /tmp/bluemap-ae2-gallery.XXXXXX)"
cleanup() {
  rm -rf -- "$archive_temp"
}
trap cleanup EXIT
...
(
  ...
  LC_ALL=C find . -type f -printf '%P\n' | LC_ALL=C sort |
    zip -q -X -9 "$archive_temp/ae2-s1-gallery.zip" -@
)

unzip -tq "$archive_temp/ae2-s1-gallery.zip"
cp "$archive_temp/ae2-s1-gallery.zip" "$output_path"
