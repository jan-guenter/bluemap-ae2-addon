...
public final class BlueMap522Adapter {
    ...
    public static synchronized boolean install() {
        ...
        if (!canRegisterExact(BlockEntityType.REGISTRY, BLOCK_ENTITY_TYPE)
                || !canRegisterExact(BlockRendererType.REGISTRY, RENDERER_TYPE)
                || !canRegisterExact(ResourcePack.Extension.REGISTRY, RESOURCE_EXTENSION_TYPE)) {
            return disableForCollision();
        }

        boolean driveDtoEligible = canRegisterExact(
                BlockEntityType.REGISTRY,
                DRIVE_BLOCK_ENTITY_TYPE
        );
        boolean driveRendererEligible = canRegisterExact(
        ...
        boolean extendedDriveDtoEligible = canRegisterExact(
                BlockEntityType.REGISTRY,
                EXTENDED_DRIVE_BLOCK_ENTITY_TYPE
        );
        boolean extendedDriveRendererEligible = canRegisterExact(
                BlockRendererType.REGISTRY,
        ...
        boolean quartzGlassRendererEligible = canRegisterExact(
                BlockRendererType.REGISTRY,
        ...
        boolean craftingMonitorDtoEligible = canRegisterExact(
                BlockEntityType.REGISTRY,
                CRAFTING_MONITOR_BLOCK_ENTITY_TYPE
        );
        boolean craftingRendererEligible = canRegisterExact(
                BlockRendererType.REGISTRY,
        ...
        boolean quantumBridgeRendererEligible = canRegisterExact(
                BlockRendererType.REGISTRY,
        ...
        boolean m3CompletionPaintDtoEligible = canRegisterExact(
                BlockEntityType.REGISTRY,
                M3_COMPLETION_PAINT_BLOCK_ENTITY_TYPE
        );
        boolean m3CompletionRendererEligible = canRegisterExact(
                BlockRendererType.REGISTRY,
        ...
        if (!registerExact(BlockEntityType.REGISTRY, BLOCK_ENTITY_TYPE)) {
            return disableForCollision();
        }
        boolean driveDtoOwned = driveDtoEligible
                && registerExact(BlockEntityType.REGISTRY, DRIVE_BLOCK_ENTITY_TYPE);
        ...
        boolean driveRendererOwned = driveRendererEligible
                && registerExact(BlockRendererType.REGISTRY, DRIVE_RENDERER_TYPE);
        ...
        boolean craftingRendererOwned = craftingRendererEligible
                && registerExact(BlockRendererType.REGISTRY, CRAFTING_RENDERER_TYPE);
    ...
    static <T extends Keyed> boolean registerExact(Registry<T> registry, T candidate) {
        T existing = registry.get(candidate.getKey());
        if (existing == null) {
            // BlueMap 5.22 returns an inverted boolean from Registry.register.
            // The only accepted result is an identity read-back.
    ...
    static boolean registerQuantumBridgeRendererExact(
        ...
        if (registerExact(registry, QUANTUM_BRIDGE_RENDERER_TYPE)) {
            return true;
        }
    ...
    static boolean registerM3CompletionRendererExact(
        ...
        if (registerExact(registry, M3_COMPLETION_RENDERER_TYPE)) {
            return true;
        }
    ...
    static <T extends Keyed> boolean canRegisterExact(Registry<T> registry, T candidate) {
        T existing = registry.get(candidate.getKey());
